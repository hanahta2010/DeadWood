...
class GameMaster extends Deadwood {
  ...
  public ArrayList<Card> cards = CardParse.CP();
...
// Tell whose turn it's on and their information
  public void who() {
  ...
  public boolean move(String to) {
	  ...
	  if (current_player.getRole() != null) {
		  ...
		  if (currentRoom.isAdjacent(to)) {
			  ...
			  System.out.println(current_player.getName() + " are moved to: " + current_player.getPosition().getName());
			  System.out.println();
			  return true;
		  } else {
			  return false;
	    ...
	    return false;
		  ...
		  if(extraRole != null){
			  if (extraRole.getIsAvailable() && (current_player.getRank() >= extraRole.getRank())) {
				  ...
				  System.out.println(current_player.getName() + " are trying to work as " + role);
				  System.out.println();
				  return true;
			  } else {
...
// Day has end  
  public boolean dayEnd() {
	  ...
	  if(getDay() < maxDays) {
		  dayStart();
		  System.out.println("Now, player should go back to the trailer!");
		  //ded.board.beginday();
		  return false;
	  } 
	  for (int i = 0; i < playerList.size(); i++) {

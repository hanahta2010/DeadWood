...
//----------------------------------------------------------------------------------------------------------------\
	...
	static public void refreshResults(String comment){
		...
		dv.stats.repaint();
		dv.mainFrame.repaint();		
		if (dead.getDay() != theDay) {
			theDay++;

		}
		String winnerName;
		
		if (dead.gameOver() == false) {
	...
	public static void main(String[] args)
	  {
		...
		int count = 0;
		for(Map.Entry<String, Room> entry : dead.getRooms().entrySet()){
			System.out.println(entry.getKey());			
			if (!entry.getKey().equals("trailer") && !entry.getKey().equals("office")) {
				deckorder[count] = entry.getValue().getCard().getScene();
				count++;
			} 

		}
		for (int i = 10; i<40; i++) {
			deckorder[i] = dead.cards().get(i-10).scene;	
		}
